# -*- coding: utf-8 -*-
# This code is adapted from reslib 3.4c
...
class FitConv(object):
    ...
    def fitwithconv(self, exp=None, sqw=None, prefactor=None, hkle=None, Iobs=None, dIobs=None, param=None, paramfixed=None, 
        ...
        #----------------------------------------------------------------------------------------------------------------
        [chisq, alpha, beta]    =    self.marqit(self.param,ivar)
        ...
        while ( np.abs(chisq_old-chisq)  >  tol*chisq ):
            ...
            pt   =  self.param + dpa                                               # new trial parameters
            ...
            if ( chisq_trial > chisq ):                                       #chisq increases ?
                ...
                self.param   = pt                                           #update parameters
                nit       = nit + 1                                             #call it an iteration
                [chisq, alpha, beta] = self.marqit(self.param,ivar)                #recalculate alpha,beta,chi-squared
                print(' {0:8d} {1:13f} {2:13f} {3:13f} {4:13f}  '.format(nit, self.fitcount, chisq/DF, lamda, round((time.time()-t)*10)/10)) 
                #print(self.param)  
        ...
        #----------------------------------------------------------------------------------------------------------------
        if (nargout>=8):
            sim = self.exp.ResConv(sqw=self.sqw, pref=self.prefactor, nargout=2, hkle=self.hkle, METHOD=self.method, ACCURACY=self.accuracy, p=self.param)
            self.fitcount  = self.fitcount + 1
   
        if (nargout==9):
            details = {"chisq": chisq,"Ndata": len(Iobs), "Npar": len(self.param), "Nva": len(ivar), "DF": DF, "C": C, "final_lamda": lamda}
        ...
        #for index, par in enumerate(param):
            #print('{0:6d} \t {1:10f} {2:12f}'.format(index, par, dpa[index]))
            
        
        return [self.param, dpa, chisqN, sim, CN, PQ, nit, kvg, details]
