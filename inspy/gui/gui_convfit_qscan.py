...
def PrefDemoMn2(H, K, L, W, EXP, p):
    ...
    prefactor[1, :] = 0 #ff ** 2.0 * poly * intensity
    prefactor[2, :] = 0 #ff ** 2.0 * polz * intensity
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        uic.loadUi(os.path.join(os.path.dirname(__file__), 'ui\\ResConFitQScan.ui'), self)
        ...
        self.param       = np.array([1.0,1, 1, 1, 1, 1, 1])
        self.param_fixed = np.array([1,  1, 1, 1, 1, 1, 1])
        self.data        = []
        self.data_start  = 0
        self.data_end    = 0
    ...
    def initData(self):
        ...
        sim_inital  = self.instrument.ResConv(sqw=SqwQScanTwoPeaks, pref=PrefDemoMn2, nargout=2, hkle=[H,K,L,W], METHOD='fix', ACCURACY=None, p=self.param)
    ...
    def fitData(self):
        ...
        [H, K, L, W, Iobs, dIobs] = self.data[:,self.data_start:self.data_end]
        fitter     =    FitConv(self.instrument,SqwQScanTwoPeaks,PrefDemoMn2,[H,K,L,W],Iobs, dIobs)

        [final_param, dpa, chisqN, sim, CN, PQ, nit, kvg, details] = fitter.fitwithconv(self.instrument,SqwQScanTwoPeaks,PrefDemoMn2,[H,K,L,W],Iobs, dIobs, param=self.param, paramfixed=self.param_fixed)
