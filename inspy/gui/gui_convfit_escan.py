...
class MainWindow(QMainWindow):
    ...
    def initData(self):
        ...
        sim_inital = self.instrument.ResConv(sqw=SqwDemo, pref=PrefDemoFF, nargout=2, hkle=[H,K,L,W], METHOD='fix', ACCURACY=[5,5], p=self.param)
    ...
    def fitData(self):
        ...
        dIobs= np.sqrt(Iobs)
        fitter     =    FitConv(self.instrument,SqwDemo,PrefDemoFF,[H,K,L,W], Iobs, dIobs)

        [final_param, dpa, chisqN, sim, CN, PQ, nit, kvg, details] = fitter.fitwithconv(self.instrument,SqwDemo,PrefDemoFF,[H,K,L,W], Iobs, dIobs, param=self.param, paramfixed=self.param_fixed)
