...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        uic.loadUi(os.path.join(os.path.dirname(__file__), 'ui\\ResConFitEScanV2.ui'), self)
    ...
    def load_instrument(self):
        ...
        str_param_fixed=self.fit_param_fixed.text()
        self.param_fixed=[ int(ii) for ii in str_param_fixed.split() if ii.isdigit()]
    ...
    def loadData(self):
        fname = QFileDialog.getOpenFileName(self, "open file", "", "data file(*.*)")
        #print(fname)
        if  not ( fname[0] == '') :
            self.fit_filepath.setText(fname[0])
            with open(fname[0], 'r') as f:
                self.data=np.loadtxt(f, unpack=True)
            [H, K, L, W, Iobs, dIobs]=self.data
            self.data_start=0
    ...
    def initData(self):
        ...
        print(W)
        sim_inital = self.instrument.ResConv(sqw=SqwDemo, pref=PrefDemo, nargout=2, hkle=[H,K,L,W], METHOD='fix', ACCURACY=[5,5], p=self.param)
        #[H, K, L, W, Iobs, dIobs]=self.data
        self.dplot.axes.clear()
        self.dplot.axes.plot(W,Iobs, "bo", W,sim_inital, "g-")
        self.dplot.fig.canvas.draw_idle()

    
    def fitData(self):
        [H, K, L, W, Iobs, dIobs] = self.data[:,self.data_start:self.data_end]
        fitter     =    FitConv(self.instrument,SqwDemo,PrefDemo,[H,K,L,W],Iobs, dIobs)

        [final_param, dpa, chisqN, sim, CN, PQ, nit, kvg, details] = fitter.fitwithconv(self.instrument,SqwDemo,PrefDemo,[H,K,L,W],Iobs, dIobs, param=self.param, paramfixed=self.param_fixed)
        
        #final = self.instrument.ResConv(sqw=SqwDemo, pref=PrefDemo, nargout=2, hkle=[H,K,L,W], METHOD='fix', ACCURACY=None, p=final_param)

        self.dplot.axes.plot(W,sim, "r-")
        self.dplot.fig.canvas.draw_idle()
        str_output="The fitted parameters:\n"
        parlist=["En1     ","En2     ","Ratio   ","Gamma1","Gamma2", "Intensity","bg      ", "temp    " ]
    ...
    def ConvolutedInelasticPeakProfile(self, parameters,fixedparlist, hkle, Iobs, dIobs):
        [H, K, L, W, Iobs, dIobs]=self.data
    
        self.instrument.calc_resolution([H,K,L,W])
 
        fitter = FitConv(self.instrument,SqwDemo,PrefDemo,[H,K,L,W],Iobs, dIobs)

        fitter.fitwithconv(self.instrument,SqwDemo,PrefDemo,[H,K,L,W],Iobs, dIobs, param=self.param, paramfixed=self.param_fixed)
        
        conv=self.instrument.ResConv(SqwDemo,PrefDemo,nargout=2,hkle=hkle,METHOD='fix', ACCURACY=[5,5], p=param)
